...
def main(timestamp_line: str, buses_str: str):
    ...
    buses = sorted([int(bus_id) for bus_id in buses_str.split(",") if bus_id != "x"])

    best_bus = buses[0]
    min_wait = buses[-1]
    for bus_period in buses:
        to_wait = bus_period - timestamp % bus_period

        if to_wait == bus_period:
            # bus departs precisely on timestamp
            return 0
